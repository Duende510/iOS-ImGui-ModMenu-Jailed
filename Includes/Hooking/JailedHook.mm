...
void* find_module_by_path(char* machoPath)
{
    ...
    for (int i = 0; i < _dyld_image_count(); i++) {
        ...
        NSString* imagePath = [NSString stringWithUTF8String:fpath];

        if ([path isEqualToString:imagePath]) {
            //showWarningPopup(@"Module Found", [NSString stringWithFormat:@"Found module at: %s", fpath]);
            //showWarningPopup(@"Base Found", [NSString stringWithFormat:@"Base at: %p", baseaddr]);
            return baseaddr;
        }
    }
    showWarningPopup(@"Module Not Found", [NSString stringWithFormat:@"Module not found for path: %@", path]);
    return NULL;
}
...
void create_hook_block(void* base, uint64_t vaddr, void* replacement) {
    struct segment_command_64* text_seg = NULL;
    struct segment_command_64* data_seg = NULL;

    struct mach_header_64* header = (struct mach_header_64*)base;

    // Iterate over the load commands to find the necessary segments
    struct load_command* lc = (struct load_command*)((UInt64)header + sizeof(*header));
    for (int i = 0; i < header->ncmds; i++) {
        if (lc->cmd == LC_SEGMENT_64) {
            struct segment_command_64* seg = (struct segment_command_64*)lc;
    ...
    if (!text_seg || !data_seg) {
        log(@"Cannot find __HOOK_TEXT or __HOOK_DATA segments!");
        return; // Exit if segments are not found
    }

    StaticInlineHookBlock* hookBlock = (StaticInlineHookBlock*)((uint64_t)header + va2rva(header, data_seg->vmaddr));

    for (int i = 0; i < STATIC_HOOK_CODEPAGE_SIZE / sizeof(StaticInlineHookBlock); i++) {
        if (hookBlock[i].hook_vaddr == 0) { // Assuming 0 means uninitialized
            hookBlock[i].hook_vaddr = vaddr;
            hookBlock[i].target_replace = replacement;
            log(@"Created hook block for address: %llX", vaddr);
            return;
        }
    }

    log(@"No available hook blocks to create a new one.");
}
...
{
    void* moduleBase = find_module_by_path(machoPath);
    //showWarningPopup(@"Base Found", [NSString stringWithFormat:@"Base at: %p", moduleBase]);
    if (!moduleBase) {
        showWarningPopup(@"Error", @"Cannot find module!");
        return NULL;
    }

    // Try to find the existing hook block.
    StaticInlineHookBlock* hookBlock = find_hook_block(moduleBase, vaddr);
    if (!hookBlock) {
        log(@"Hook block not found for address: %llX, creating a new one.", vaddr);
/*
        // Create a new hook block (ensure you have this function defined)
        create_hook_block(moduleBase, vaddr, replacement);

        // After creation, try to find it again.
        hookBlock = find_hook_block(moduleBase, vaddr);
        if (!hookBlock) {
            showWarningPopup(@"Error", @"Cannot find hook block after creation!");
            return NULL;
        }*/
        return NULL;
    }

    // Now set the replacement target.
    hookBlock->target_replace = replacement;

    // Return the original virtual address of the hook block.
    //showWarningPopup(@"Success", @"It worked");
    return (void*)((uint64_t)moduleBase + hookBlock->original_vaddr);
}
